...
class Craft(Bash):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        if self.distro == (Dist.UBUNTU, Dist.V16_04):
            ...
            raise PlatformError(
                "Craft3 dependencies have not been determined yet for this platform: {}".format(
                    self.distro
                )
            )
    ...
    def post_install(self) -> None:
        ...
        if not self.args.dry_run:
            if os.path.exists(craft_dir) and os.listdir(craft_dir):
                error(
                    "Craft dir is not empty. ({})".format(craft_dir), self.args.dry_run
                )
        ...
        self.run(
            "sudo -u www-data composer create-project --no-ansi --remove-vcs --no-interaction craftcms/craft {}".format(
                craft_dir
            )
        )
        ...
        self.run(
            """sudo -u www-data php {craft_dir}/craft install/craft --interactive=0 \
            --email={email} \
            --username={username} \
            --password={password} \
            --siteName={sitename} \
